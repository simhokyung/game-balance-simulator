# ⚔️ 우테코 오픈미션 - Game Balance Simulator
...
## 📑 목차
1. [프로젝트 소개](#-우테코-오픈미션---game-balance-simulator)
2. [구현된 기능 개요](#-구현된-기능-개요)
3. [입출력 명세](#-입출력-명세)
4. [전투 규칙 요약](#-전투-규칙-요약)
    - [1) 속도 게이지 기반 턴 시스템](#1-속도-게이지-기반-턴-시스템)
    - [2) 기본 데미지 공식](#2-기본-데미지-공식)
    - [3) 스킬 훅](#3-스킬-훅)
    - [4) 전투 종료 조건](#4-전투-종료-조건)
5. [전투 캐릭터 설명](#-내장-프리셋-1대1-밸런스형-전투-캐릭터-6종)
    - [Bruiser](#1-bruiser--맞을수록-강해지는-근접-브루저)
    - [Assassin](#2-assassin--짧은-턴-안에-승부-보는-폭딜-암살자)
    - [Tank](#3-tank--장기전-방패--반격-탱커)
    - [Speedster](#4-speedster--턴-이득과-회피를-노리는-스피드형)
    - [Sustainer](#5-sustainer--흡혈과-재생으로-버티는-장기전형)
    - [Hybrid](#6-hybrid--상황에-따라-공격방어-모드를-바꾸는-만능형)
6. [구현 기능 목록](#-구현-기능-목록)
7. [예외 처리 규칙](#-예외-처리-규칙)
8. [테스트 케이스 (JUnit5 + AssertJ)](#-테스트-케이스-junit5--assertj)
9. [프로젝트 구조](#-프로젝트-구조)
10. [커밋 컨벤션 (AngularJS Style)](#-커밋-컨벤션-angularjs-style)
11. [개발 진행 방식](#-개발-진행-방식)
12. [확장 가능성](#-확장-가능성)
13. [개발 환경](#-개발-환경)
14. [참고 자료](#-참고-자료)
...
마지막에는 각 캐릭터들 간의 승률 확인 여부를 선택한다. 
...
번호를 입력하세요 (1-2):
```
`1`을 선택하면 **캐릭터 vs 캐릭터 승률 테이블**이 출력된다. (아래 예시)
